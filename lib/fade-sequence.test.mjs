...
//
//   node --test lib/fade-sequence.test.mjs
//
// buildFadeFilterGraph is re-implemented here rather than imported, because
// lib/ffmpeg.ts is TypeScript that node --test cannot load without a build
// step. Keep the two in sync — see lib/ffmpeg.ts buildFadeFilterGraph.
...
test("the filtergraph gives every seam its own transition", async () => {
  const { buildFadeFilterGraph } = await import("./ffmpeg.ts");
  const clip = { start: 0, end: 10, duration: 10, hasAudio: true, volume: 1, crop: { x: 0.5, y: 0.5 } };
  const { chains, outputDuration } = buildFadeFilterGraph([clip, clip, clip], {
    transitions: [
  ...
  const withCut = buildFadeFilterGraph([clip, clip], { transitions: [null, { type: "cut", duration: 1 }] });
...
test("the filtergraph renders blank gaps as black video and silence", async () => {
  ...
  const { chains, extraInputs, outputDuration } = buildFadeFilterGraph([clip, second], {
...
test("caption is never baked into the video — it's post text, not a render option", async () => {
  ...
  assert.doesNotMatch(ffmpegSource, /captionPng/, "buildFadeFilterGraph/FadeSequenceOptions must not accept a caption overlay again");
...
test("Captions-timeline text overlays composite with a time-gated overlay filter, not drawtext", async () => {
  const { buildFadeFilterGraph } = await import("./ffmpeg.ts");
  const clip = { start: 0, end: 10, duration: 10, hasAudio: true, volume: 1, crop: { x: 0.5, y: 0.5 } };
  const { chains, extraInputs } = buildFadeFilterGraph([clip], {
    width: 1080,
    height: 1920,
  ...
  const none = buildFadeFilterGraph([clip], {});
...
test("opening and closing fade to/from a synthesized black clip without changing duration", async () => {
  ...
  const plain = buildFadeFilterGraph([clip], { transitions: [{ type: "cut", duration: 0.5 }] });
  ...
  const opened = buildFadeFilterGraph([clip], { transitions: [{ type: "wipeleft", duration: 0.7 }] });
  ...
  const closed = buildFadeFilterGraph([clip], { closing: { type: "circleclose", duration: 0.4 } });
  ...
  const clampedOpen = buildFadeFilterGraph([clip], { transitions: [{ type: "fade", duration: 2 }] });
  ...
  const clampedShort = buildFadeFilterGraph([shortClip], { transitions: [{ type: "fade", duration: 2 }] });
